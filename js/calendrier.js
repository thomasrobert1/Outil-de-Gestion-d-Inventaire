...
function rendreCalendrier() {
  ...
  const casesHtml = cases.map(c => {
    ...
    const resasDuJour = TOUTES_RESERVATIONS.filter(r => r.dateDebut <= c.dateStr && (!r.dateFin || r.dateFin >= c.dateStr));
    const evenementsHtml = resasDuJour.slice(0, 3).map(r =>
      `<div class="evenement-resa" data-resa-id="${r.id}" title="${escapeAttr(r.composantReference)} — ${escapeAttr(r.personne)}">${escapeHtml(r.composantReference)}</div>`
    ).join("");
...
function ouvrirDetailJour(dateStr) {
  ...
  const resasDuJour = TOUTES_RESERVATIONS.filter(r => r.dateDebut <= dateStr && (!r.dateFin || r.dateFin >= dateStr));
  ...
  const corps = resasDuJour.length === 0
    ...
    : `<div class="liste-historique">${resasDuJour.map(r => `
        <div class="ligne-historique" style="grid-template-columns:1fr auto;">
          <div>
...
function ouvrirModaleReservation() {
  ...
  document.getElementById("r-date-debut").value = dateSelectionnee || aujourdHui;
  document.getElementById("r-date-fin").value = "";
  document.getElementById("r-duree").value = "1";
...
document.querySelectorAll('[data-fermer-modale="modale-reservation"]').forEach(btn => {
  btn.addEventListener("click", () => { document.getElementById("modale-reservation").hidden = true; });
});
...
document.getElementById("btn-confirmer-reservation").addEventListener("click", async () => {
  ...
  try {
    ...
    btn.disabled = false;
    btn.textContent = "Enregistrer la réservation";
  }
});
